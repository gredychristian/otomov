...
<body class="antialiased bg-slate-50">
    <div class="relative min-h-screen">
        ...
        {{-- BAGIAN HEADER YANG PANJANG DIGANTI DENGAN SATU BARIS INI --}}
        @include('layouts.navigation')

        <main class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 mb-8">Katalog</h1>

            {{-- Galeri Produk --}}
            <div class="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
                @forelse ($products as $product)
                    {{-- Awal dari Kartu Produk --}}
                    <a href="{{ route('products.show', $product->slug) }}"
                        class="group block bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300">
                        {{-- Bagian Gambar (Kotak) --}}
                        <div class="aspect-square w-full overflow-hidden rounded-t-lg bg-gray-100">
                            ...
                            @else
                                {{-- Placeholder baru jika tidak ada gambar, menggunakan logo bus Anda --}}
                                <div class="h-full w-full flex items-center justify-center p-8 bg-gray-100">
                                    <svg class="h-full w-full text-gray-300" viewBox="0 0 50 50"
                    ...
                    {{-- Akhir dari Kartu Produk --}}
                @empty
                    <p class="col-span-full text-center text-gray-500">Belum ada kendaraan yang tersedia.</p>
                @endforelse
            </div>
        </main>
